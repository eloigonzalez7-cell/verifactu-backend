...
function ensureSingleVAT(lines) {
  const distinctRates = [...new Set(lines.map((line) => line.iva))];
  if (distinctRates.length !== 1) {
    throw new Error("All invoice lines must share the same IVA rate for this simplified builder");
  }
...
export async function buildInvoiceXml(invoice, signature) {
  ...
  const baseTotal = lineas.reduce((acc, line) => acc + Number(line.base || 0), 0);
  const quotaTotal = lineas.reduce(
    (acc, line) => acc + Number(line.base || 0) * Number(line.iva || 0) / 100,
    0
  );
